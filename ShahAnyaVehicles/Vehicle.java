...
public abstract class Vehicle extends SuperSmoothMover
{
    ...
    protected boolean changingLanes;
    protected int laneY;
    protected GreenfootSound speedUp;
    ...
    public Vehicle (VehicleSpawner origin) {
        ...
        speedUp = new GreenfootSound("speedUpSound.mp3");
    ...
    public void drive() 
    {
        ...
        if (otherVehicleSpeed >= 0 && otherVehicleSpeed < maxSpeed){ // Vehicle ahead is slower?
            ...
            if(isTouching(ShootingStar.class) && !(this instanceof Asteroid)) {
                speed = 1.5;
            }
            else {
    ...
    /**
     * Checks if vehicles are present in the lane at the specified y coordinate.
     * Returns true if the lane is clear. Returns false if vehicle is present.
     */
    public boolean checkAdjacentLane(int y) {
        LaneSpawnCheck test = new LaneSpawnCheck(getImage().getWidth(), getImage().getHeight(), this);
        getWorld().addObject(test, getX(), y);
        return !(test.vehiclePresent());
    }
    
    /**
     * Based on the current lane number, method checks for vehicles in adjacent lanes. It updates the current lane number if a 
     * clear adjacent lane is found. If no clear lane is found it returns -1, meaning that no lane change can be made.
     */
    public int checkLanes() {
        if(myLaneNumber == 0 || myLaneNumber == 2) {
            if(checkAdjacentLane(new VehicleWorld().getLaneY(1))) {
                myLaneNumber = 1;
                return new VehicleWorld().getLaneY(1);
            }
        }
        if(myLaneNumber == 1) {
            if(checkAdjacentLane(new VehicleWorld().getLaneY(0))) {
                myLaneNumber = 0;
                return new VehicleWorld().getLaneY(0);
            }
            if(checkAdjacentLane(new VehicleWorld().getLaneY(2))) {
                myLaneNumber = 2;
                return new VehicleWorld().getLaneY(2);
            }
        }
        if(myLaneNumber == 3 || myLaneNumber == 5) {
            if(checkAdjacentLane(new VehicleWorld().getLaneY(4))) {
                myLaneNumber = 4;
                return new VehicleWorld().getLaneY(4);
            }
        }
        if(myLaneNumber == 4) {
            if(checkAdjacentLane(new VehicleWorld().getLaneY(3))) {
                myLaneNumber = 3;
                return new VehicleWorld().getLaneY(3);
            }
            if(checkAdjacentLane(new VehicleWorld().getLaneY(5))) {
                myLaneNumber = 5;
                return new VehicleWorld().getLaneY(5);
            }
        }
        return -1;
    }
    ...
    /**
     * Method that updates my postion based on the target lane and my current position if a lane change is possible.
     */
    public void changeLanes(int lane) {
        int changedY = lane - yOffset; // difference between target lane and current y coordinate
        int dir = Integer.signum(changedY - getY()); // calculates the sign of difference to see if I need to move up or down
        if(dir == -1 && getY() > changedY || dir == 1 && getY() < changedY) {
            setLocation(getX(), getY() + (dir * 2));
        }
        else {
            changingLanes = false;
        }
